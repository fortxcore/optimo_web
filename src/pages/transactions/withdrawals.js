...
function Withdrawals() {
  ...
  const { user } = useAuth()
  const [data, loading, error] = useCollection(firebase.firestore().collection(`uData/${user.uid}/transactions`).where('type', "==", "WITHDRAW").orderBy('id', 'desc').limit(resultsPerPage), {})

  return (
    <>
      <PageTitle>Withdrawals</PageTitle>
      <SectionTitle>Your Withdrawals</SectionTitle>
